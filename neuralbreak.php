...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* --- PATHFINDER GRID STYLES --- */
        .pathfinder-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px; width: 100%; max-width: 220px; margin: 20px auto; }
        .node { aspect-ratio: 1/1; background: rgba(255, 255, 255, 0.05); border: 1px solid rgba(255, 255, 255, 0.2); border-radius: 8px; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all 0.3s ease; }
        .node i { font-size: 1.5rem; color: rgba(255, 255, 255, 0.3); transition: transform 0.3s cubic-bezier(0.4, 0, 0.2, 1); }
        .node:hover { background: rgba(255, 255, 255, 0.1); }
        .node.active i { color: var(--brand-gold); text-shadow: 0 0 10px var(--brand-gold); }
        .node.connected { border-color: var(--brand-green); background: rgba(74, 222, 128, 0.1); }
        .node.connected i { color: var(--brand-green); text-shadow: 0 0 10px var(--brand-green); }
        ...
        <div class="game-column-right">
            <div class="glass-panel" style="flex: 1.5; text-align: center;">
                ...
                <div class="pathfinder-grid" id="path-grid">
                    <?php for($i=0; $i<9; $i++): ?>
                        <div class="node" onclick="rotateNode(this, <?php echo $i; ?>)"><i class="fas fa-route"></i></div>
                    <?php endfor; ?>
                </div>
                <div id="logic-status" style="color: var(--brand-gold); font-size: 0.7rem; font-weight: bold;">CIRCUIT FRAGMENTED</div>
                <button onclick="resetLogic()" style="background: none; border: 1px solid var(--brand-green); color: var(--brand-green); font-size: 0.6rem; padding: 5px 10px; margin-top: 15px; cursor: pointer; border-radius: 4px; text-transform: uppercase;">Re-Randomize</button>
            </div>
    ...
    <script>
        ...
        function drawElements() {
            ...
            Object.keys(ladders).forEach(start => {
                const s = getCoords(start), e = getCoords(ladders[start]);
                const line = document.createElementNS("http://www.w3.org/2000/svg", "line");
                line.setAttribute("x1", s.x); line.setAttribute("y1", s.y);
                line.setAttribute("x2", e.x); line.setAttribute("y2", e.y);
                line.setAttribute("stroke", "rgba(255,255,255,0.7)");
                line.setAttribute("stroke-width", "1.5"); line.setAttribute("stroke-dasharray", "1, 2");
                svg.appendChild(line);
            });
            Object.keys(snakes).forEach(head => {
                const s = getCoords(head), e = getCoords(snakes[head]);
                const path = document.createElementNS("http://www.w3.org/2000/svg", "path");
                path.setAttribute("d", `M ${s.x} ${s.y} Q ${(s.x + e.x) / 2 + 10} ${(s.y + e.y) / 2} ${e.x} ${e.y}`);
                path.setAttribute("stroke", "var(--snake-red)"); path.setAttribute("stroke-width", "2");
        ...
        function executeRoll() {
            if (isRolling) return;
            isRolling = true;
            const dice = document.getElementById('dice'), roll = Math.floor(Math.random() * 6) + 1;
        ...
        const winningPattern = [90, 0, 270, 180, 90, 0, 90, 180, 270];

        function rotateNode(el, index) {
            rotations[index] = (rotations[index] + 90) % 360;
            el.querySelector('i').style.transform = `rotate(${rotations[index]}deg)`;
            el.classList.add('active');
            checkPath();
        }

        function checkPath() {
            let win = rotations.every((val, i) => val === winningPattern[i]);
            if (win) {
                ...
                document.querySelectorAll('.node').forEach(n => n.classList.add('connected'));
            }
        }
        ...
        // --- NEURAL DRIFT LOGIC (STAYS THE SAME) ---
        ...
        let driftScore = 0, driftActive = false, driftPlayer = { x: 0, y: 0, size: 8 }, obstacles = [], powerups = [], driftAnimationId;
        ...
        driftContainer.addEventListener('mousemove', (e) => {
            ...
            driftPlayer.x = Math.max(5, Math.min(driftCanvas.width - 5, e.clientX - rect.left));
        });

        driftOverlay.addEventListener('click', () => {
            driftOverlay.style.display = 'none'; driftScore = 0; driftScoreEl.innerText = driftScore;
            obstacles = []; powerups = []; driftActive = true; initDriftGame(); animateDrift();
        });

        function animateDrift() {
            if (!driftActive) return;
            driftCtx.clearRect(0, 0, driftCanvas.width, driftCanvas.height);
            driftCtx.shadowBlur = 15; driftCtx.shadowColor = '#4ade80'; driftCtx.fillStyle = '#4ade80';
            driftCtx.beginPath(); driftCtx.arc(driftPlayer.x, driftPlayer.y, driftPlayer.size, 0, Math.PI * 2); driftCtx.fill();
            if (Math.random() < 0.05) obstacles.push({ x: Math.random() * driftCanvas.width, y: -10, speed: 2 + Math.random() * 3, size: 10 });
            if (Math.random() < 0.01) powerups.push({ x: Math.random() * driftCanvas.width, y: -10, speed: 2 });
            driftCtx.shadowBlur = 5; driftCtx.shadowColor = '#ff4757'; driftCtx.fillStyle = '#ff4757';
            obstacles.forEach((ob, i) => {
                ob.y += ob.speed; driftCtx.fillRect(ob.x, ob.y, ob.size, ob.size);
                if (Math.hypot(driftPlayer.x - ob.x, driftPlayer.y - ob.y) < driftPlayer.size + 5) driftGameOver();
                if (ob.y > driftCanvas.height) obstacles.splice(i, 1);
            });
            driftCtx.shadowColor = '#ffc629'; driftCtx.fillStyle = '#ffc629';
            powerups.forEach((p, i) => {
                p.y += p.speed; driftCtx.beginPath(); driftCtx.arc(p.x, p.y, 5, 0, Math.PI * 2); driftCtx.fill();
                if (Math.hypot(driftPlayer.x - p.x, driftPlayer.y - p.y) < driftPlayer.size + 5) {
                    driftScore += 10; driftScoreEl.innerText = driftScore; powerups.splice(i, 1);
                }
                if (p.y > driftCanvas.height) powerups.splice(i, 1);
            });
            driftAnimationId = requestAnimationFrame(animateDrift);
        }

        function driftGameOver() {
            driftActive = false; cancelAnimationFrame(driftAnimationId);
            driftOverlay.style.display = 'flex';
            driftOverlay.innerHTML = `<span style="color: #ff4757; font-weight: bold;">CONNECTION LOST<br>BIT-RATE: ${driftScore}<br>CLICK TO REBOOT</span>`;
        }

        window.onload = () => { movePlayer(1); drawElements(); initDriftGame(); resetLogic(); };
